...
export default function UploadPayroll() {
  ...
  const [visitUidsCreated, setVisitUidsCreated] = useState<number | null>(null);
  ...
  const handleUpload = async () => {
    ...
    setVisitUidsCreated(null); // reset count
    ...
    try {
      ...
      const { res, result } = await apiFetch(
        "http://localhost:8002/api/upload-visit-file",
        {
          method: "POST",
          body: formData,
        }
      ...
      const skipped = result.skipped?.map(
        (id: number) => `Note ID ${id} already exists`
      ) ?? [];
      ...
      setVisitUidsCreated(result.visit_uids_created ?? null);
  ...
  return (
    <div className="h-full w-full bg-gray-100 p-6 flex flex-col">
      <div className="w-full flex-grow bg-white shadow rounded-lg p-6 flex flex-col">
        ...
        {/* ✅ Success detail for visit_uids_created */}
        {visitUidsCreated !== null && (
          <div className="mt-6 p-4 border border-green-400 bg-green-50 rounded text-sm text-green-800">
            ✅ Created {visitUidsCreated} new visit_uid values
          </div>
        )}
